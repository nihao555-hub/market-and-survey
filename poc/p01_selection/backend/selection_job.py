...
## 📊 图表输出（重要！让报告更直观）
在分析数据后，请使用 ```chart 代码块输出 ECharts 图表，前端会自动渲染为交互式可视化。
格式示例：

```chart
{{"type":"bar","title":"品类价格分布","xAxis":["$10-20","$20-30","$30-50","$50+"],"series":[{{"name":"商品数量","data":[15,28,22,8]}}]}}
```

```chart
{{"type":"line","title":"近7天销量趋势","xAxis":["6/14","6/15","6/16","6/17","6/18","6/19","6/20"],"series":[{{"name":"日均销量","data":[120,135,142,128,155,168,172]}}]}}
```

```chart
{{"type":"pie","title":"竞品市场份额","xAxis":["品牌A","品牌B","品牌C","其他"],"series":[{{"name":"份额","data":[35,25,20,20]}}]}}
```

```chart
{{"type":"radar","title":"候选品多维对比","xAxis":["价格竞争力","评分","销量","物流时效","利润空间"],"series":[{{"name":"产品A","data":[85,92,70,60,78]}},{{"name":"产品B","data":[70,88,90,75,65]}}]}}
```

支持的图表类型：bar（柱状图）、line（折线图）、pie（饼图）、radar（雷达图）、scatter（散点图）。
**在以下场景务必输出图表**（不要只输出纯文字）：
- 价格分布分析 → bar
- 销量/评分/搜索热度趋势 → line
- 竞品市场份额/品牌集中度 → pie
- 多维度对比（价格/评分/销量/物流/利润） → radar
- BSR 排名趋势 → line
- 成本结构拆解 → pie 或 bar
- 季节性走势 → line
- 月度热度对比 → bar

每完成一个分析维度就附带图表，不要等到最终报告才一次性输出。让用户随时能看到可视化结果。
